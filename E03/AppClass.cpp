...
sf::Keyboard::Key m_savedKey;
bool m_keyWasSaved = false;
...
{
	m_uShaderProgramID = LoadShaders("Shaders//BasicColor2.vs", "Shaders//BasicColor2.fs");
...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	...
	{
		// checking if you pressed the same key as you pressed the first time (and if you saved a key)
		if (a_event.key.code == m_savedKey && m_keyWasSaved)
		{
			GLuint pressed = glGetUniformLocation(m_uShaderProgramID, "pressed");
			glUniform1f(pressed, false);
			m_keyWasSaved = false;
			return;
		}
		// if you haven't changed the color yet, save this key that was pressed
		if (m_keyWasSaved == false)
		{
			m_savedKey = a_event.key.code;
			m_keyWasSaved = true;
		}
		// changing color to its complement now that the key is pressed
		GLuint pressed = glGetUniformLocation(m_uShaderProgramID, "pressed");
		glUniform1f(pressed, true);
	}
